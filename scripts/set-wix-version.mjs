...
if (!match) {
  console.error(`set-wix-version: unsupported version "${version}"; expected X.Y.Z or X.Y.Z-rc.N`);
  process.exit(1);
}

const [, major, minor, patch, rc] = match;
const wixVersion = rc === undefined ? `${major}.${minor}.${patch}` : `${major}.${minor}.${patch}.${rc}`;
...
console.log(`set-wix-version: bundle.windows.wix.version = ${wixVersion} (from ${version})`);
